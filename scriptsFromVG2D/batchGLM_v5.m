% version 5 is designed for use with the new median filtered data using
% phase and theta head
clear
ca
d = dir('*w_phase.mat');
noProx = 1;
...
for ddd =1:length(d)
    ...
    % add phase to mechanics
    [XM_Mech_p,dm_Mech_p] = buildDesignMatrix([Mech.filtAll phase],spikevec,'deriv',0,'hist',0,'bsStim',stimBasis);
    [XM_Mech_thh,dm_Mech_thh] = buildDesignMatrix([Mech.filtAll theta_head],spikevec,'deriv',0,'hist',0,'bsStim',stimBasis);
    ...
    yHat_Mech_p = zeros(size(spikevec));
    yHat_Mech_thh = zeros(size(spikevec));
    for cross = 1:k
        ...
        %phase addition
        [bMech_p,~,stats_Mech_p] = glmfit(XM_Mech_p(idx~=cross,:),spikevec(idx~=cross),'binomial');
        [bMech_thh,~,stats_Mech_thh] = glmfit(XM_Mech_thh(idx~=cross,:),spikevec(idx~=cross),'binomial');
    ...
    r(ddd,4) = corr(rate(C)',yHat_Mech_p(C));
    r(ddd,5) = corr(rate(C)',yHat_Mech_thh(C));
    ...
    if saveTGL
        cd(saveLoc)
        save([d(ddd).name([1:end-4]) suffix '.mat'],'C','prox','med','dis','Geo','Mech','spikevec','yHat*','XM*','dm*','stimBasis','filt','r')
        cd ..
    end
