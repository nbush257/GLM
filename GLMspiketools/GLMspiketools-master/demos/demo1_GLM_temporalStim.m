...
dtStim = .001; % Bin size for stimulus (in seconds).  (Equiv to 100Hz frame rate)
dtSp = .0001;  % Bin size for simulating model & computing likelihood (must evenly divide dtStim);
nkt = 10;    % Number of time bins in stimulus filter k
...
slen = size(X,1); % Stimulus length (frames); more samples gives better fit
swid = 6;  % Stimulus width  (pixels); must match # pixels in stim filter

% Make stimulus
Stim = 2*randn(slen,swid);
% Simulate model
[tsp,sps,Itot,Istm] = simGLM(ggsim,Stim);  % run model
% tsp = find(spbool_upsamp>0);
% sps = spbool_upsamp>0;
